...
public class CompanyDao {
	// save Company
		public int addCompany(User user) {
			int status=0;
			try {
		...
		public List<User> viewCompany(){
			ArrayList<User> companyList=new ArrayList<User>();//Creating Arraylist 
			try {
				Connection con = DbConnection.getConnection();
				String sql="select userId,userName,userEmail,userPass,userImg,userPhone,redgDate,userStatus from user where roleId=3";
