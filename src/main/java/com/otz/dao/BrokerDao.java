...
public class BrokerDao {
	// save  Customer
		public int addBroker(User user) {
			int status=0;
			try {
		...
		public List<User> viewBroker() {
			ArrayList<User> brokerList=new ArrayList<User>();//Creating Arraylist 
			try {
				Connection con = DbConnection.getConnection();
				String sql="select userId,userName,userEmail,userPass,userImg,userPhone,redgDate,userStatus from user where roleId=4";
